...
### Profits & CEO Salary
The relationship between Salary and Profits doesn't have a strong linear relationship as demonstrated by the low correlation.
```{r}
...
However, after performing a logorithmic transformation on the profits variable, the relationship between the two variables shows more a linear relationship.   
```{r}
...
*Mention Companies with market cap less than 0*
```{r}
```
